...
| `SAGE_API_KEY` | `triage --sage`, `process --sage-gate` | Levanto Sage API key. `LEVANTO_API_KEY` is accepted as an alias. Required by both Sage-backed paths even on the local-subscription route. |
